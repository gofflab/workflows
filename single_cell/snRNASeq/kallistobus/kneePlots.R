...
# Merge spliced and unspliced (for snRNA-Seq analysis)
## Create intersection of cell barcodes
idx<-intersect(colnames(spliced),colnames(unspliced))
spliced_intersection <- spliced[,idx]
unspliced_intersection <- unspliced[,idx]
spliced_plus_unspliced = spliced_intersection + unspliced_intersection

write("Summarizing",stderr())
bc_rank <- barcodeRanks(spliced_plus_unspliced)

tot_count <- Matrix::colSums(spliced_plus_unspliced)

write("Making knee plot",stderr())
knee<- knee_plot(list(spliced_plus_unspliced = bc_rank)) +
        coord_flip()

pdf(file=paste0(outDir,"/spliced_plus_unspliced_knee.pdf"),width=5,height=5)
  print(knee)
dev.off()

write(paste0("Filtering for cells >",metadata(bc_rank)$inflection," UMIs"),stderr())
bcs_use <- colnames(spliced_plus_unspliced)[tot_count > metadata(bc_rank)$inflection]
# Remove genes that aren't detected
tot_genes <- Matrix::rowSums(spliced_plus_unspliced)
#genes_use <- rownames(spliced)[tot_genes > 0]
sf <- spliced_plus_unspliced[, bcs_use]

write(paste0(dim(sf)[2]," cells remain after knee filter"),stderr())

write(paste0("Writing output to ",outDir),stderr())
write("     spliced_plus_unspliced Matrix",stderr())
#spliceFile<-paste0(outDir,"/spliced.mtx")
Matrix::writeMM(sf,file.path(outDir,"spliced_plus_unspliced.mtx"))
write.table(rownames(sf),file=paste0(outDir,"/spliced_plus_unspliced.genes.txt"),row.names=FALSE,col.names=FALSE,quote=FALSE)
write.table(colnames(sf),file=paste0(outDir,"/spliced_plus_unspliced.barcodes.txt"),row.names=FALSE,col.names=FALSE,quote=FALSE)
